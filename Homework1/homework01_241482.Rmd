...
```{r barplot breast}
ggplot(data = dataf, aes(x = breast,fill=breast)) + geom_bar( alpha=0.7) +theme(axis.title.x=element_blank(),
...
By looking at the plots, we can see that the distribution of the variable `age` doesn't seem skewed, so we could replace the NAs with the mean. While, the distribution of the variable `educat` is more skewed and we will use the median to replace the NAs.
